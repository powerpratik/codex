...
from additional_strategies import (
    RandomKVCacheStrategy,
    AttentionTopStrategy,
    AttentionBottomStrategy,
    HybridNPercentStrategy
)
...
def get_enhanced_strategy(name, threshold, config):
    ...
    if name.startswith("Random"):
        # Extract parameters from name: Random(keep=0.7)
        params = {}
        if "(" in name:
            param_str = name.split("(")[1].rstrip(")")
            for p in param_str.split(","):
        ...
        keep_ratio = params.get("keep", 0.7)
        return RandomKVCacheStrategy(threshold, keep_ratio)
    
    if name.startswith("AttentionTop"):
        # Extract parameters from name: AttentionTop(keep=0.7)
        params = {}
        if "(" in name:
            param_str = name.split("(")[1].rstrip(")")
            for p in param_str.split(","):
        ...
        keep_ratio = params.get("keep", 0.7)
        return AttentionTopStrategy(threshold, keep_ratio)
    
    if name.startswith("AttentionBottom"):
        # Extract parameters from name: AttentionBottom(keep=0.7)
        params = {}
        if "(" in name:
            param_str = name.split("(")[1].rstrip(")")
            for p in param_str.split(","):
        ...
        keep_ratio = params.get("keep", 0.7)
        return AttentionBottomStrategy(threshold, keep_ratio)
    
    if name.startswith("HybridNPercent"):
        # Extract parameters from name: HybridNPercent(keep=0.7,r=0.5,a=0.3,t=0.2)
        params = {}
        if "(" in name:
            param_str = name.split("(")[1].rstrip(")")
            for p in param_str.split(","):
        ...
        keep_ratio = params.get("keep", 0.7)
        recency_weight = params.get("r", 0.5)
        attention_weight = params.get("a", 0.3)
        type_weight = params.get("t", 0.2)
        return HybridNPercentStrategy(threshold, keep_ratio, recency_weight, attention_weight, type_weight)
    ...
    for strat_name in cfg["strategies"]:
        ...
        for sample_idx, sample in enumerate(tqdm(dataset, desc=strat_name)):
            ...
            for i in range(cfg["max_gen_tokens"]):
                ...
                # For HybridNPercent, pass token_types as well
                if isinstance(strat, HybridNPercentStrategy):
                    past = cache_manager.apply_eviction_strategy(past, strat.name, attention_scores, token_types)
                else:
